...
通过百度千帆 AI PPT API 生成演示文稿，流程：获取主题 → 生成大纲 → 生成 PPT。

用法:
    python generate_ppt.py --query "PPT主题" [--style_id 0] [--tpl_id 模板ID]

参数:
    --query, -q: PPT 主题描述（必填）
    --style_id, -si: 样式 ID，默认 0
    --tpl_id, -tp: 模板 ID，不指定则随机选择
    --web_content, -wc: 额外网页内容

输出:
    流式 JSON，最终结果包含 is_end=true 和 PPT 下载链接
