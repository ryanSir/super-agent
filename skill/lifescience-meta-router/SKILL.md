...
|--------|----------|-------------|
| **⚡ Time-Bounded Aggregation** | 最近N天/周/月 + ≥2 disease domains or entity types, "past week pipeline", "recent updates across X and Y" | **SPECIAL CASE — classify this FIRST before any other intent.** Cross-domain recent updates with a time constraint. Always routes to `lifescience-general-research` + Deep-dive + Time-Bounded Aggregation Mode. NEVER Fast-track. NEVER news-only. |
...
| **Catalyst Events** | catalyst, PDUFA, data readout, AdCom, NDA submission, binary event, milestone, conference, Phase 3 readout, upcoming events | Focus on upcoming binary events and catalyst calendar |
| **Regulatory Analysis** | FDA, EMA, approval, regulatory pathway, ODD, BTD | Focus on regulatory strategy |
| **Biomarker Analysis** | biomarker, diagnostic, prognostic, companion diagnostic, biomarker patent, biomarker literature, diagnostic marker, prognostic marker, predictive marker, pharmacodynamic marker | Focus on biomarker analysis, CDx, and patent/literature investigation |
| **Clinical Outcome Analysis** | efficacy, endpoint, ORR, OS, PFS, survival, subgroup | Focus on clinical outcome data |
| **Patent Intelligence** | patent, FTO, IP, generic, biosimilar, cliff, patent moat, PTE, LOE, patent expiry, technology evolution, foundational patent | Focus on IP, patent risks, moat analysis, and technology evolution |
...
| **Clinical Outcome Analysis** | clinical comparison, drug class efficacy, competitive clinical landscape, evidence chain, clinical strategy, ORR, OS, PFS, subgroup, trial design | Focus on clinical outcomes, multi-drug clinical intelligence, and evidence chains |
| **Sequence Report** | sequence search, protein sequence, nucleotide sequence, sequence patent landscape | Focus on sequence-based patent analysis |
| **Structure Report** | structure search, SMILES, chemical structure, compound similarity, structure patent | Focus on structure-based patent analysis |
| **Multi-Domain** | Multiple entity types combined | Requires orchestration |
| **General / Ambiguous** | Open-ended overview, unclear intent, no specific angle | Route to `lifescience-general-research` |

> **Time-Bounded Aggregation detection rule**: If the query contains BOTH (a) a time expression ("最近", "past N days/weeks", "recent", "latest", "本周", "上周") AND (b) ≥2 disease domains or entity types — classify as **Time-Bounded Aggregation** immediately. Do not classify as "news monitoring", "Fast-track", or any other intent. This classification locks in: `lifescience-general-research` skill + Deep-dive mode + Time-Bounded Aggregation Mode execution (all 5 steps mandatory).
...
| Dimension | Skill |
|-----------|-------|
| Target competitive landscape, pipeline, mechanism | `lifescience-target-intelligence` |
| Specific drug characteristics, MoA, ADMET, safety | `lifescience-pharmaceuticals-exploration` |
| Disease pathophysiology, SoC, unmet needs | `lifescience-disease-investigation` |
| Company R&D pipeline, BD strategy, positioning | `lifescience-company-profiling` |
| Deal structure, licensing, M&A, valuation | `lifescience-deal-intelligence` |
| Incidence, prevalence, patient population | `lifescience-epidemiology-analysis` |
| Post-market safety signals, FAERS, disproportionality | `lifescience-pharmacovigilance` |
| Oncology variant actionability, OncoKB, TMB/MSI | `lifescience-precision-oncology` |
| GWAS hits, genetically validated targets, MR evidence | `lifescience-gwas-target-discovery` |
| Market size, pricing, reimbursement, revenue | `lifescience-commercial-analysis` |
| Upcoming binary events, PDUFA dates, data readouts, AdCom, catalyst calendar | `lifescience-catalyst-events` |
| Regulatory pathway, approval odds, FDA/EMA strategy | `lifescience-regulatory-analysis` |
| Biomarker, CDx, patient stratification, biomarker patent/literature investigation (diagnostic, prognostic, predictive, PD) | `lifescience-biomarker-analysis` |
| Clinical efficacy endpoints, safety signals, subgroup | `lifescience-clinical-outcome-analysis` |
| Patent landscape, FTO, generic/biosimilar entry | `lifescience-patent-intelligence` |
| Target biology, druggability, pipeline competitive landscape | `lifescience-target-intelligence` |
| Multi-drug or drug-class clinical intelligence, evidence chain, competitive clinical landscape | `lifescience-clinical-outcome-analysis` |
| Drug-specific patent moat, PTE, patent cliff, technology evolution, foundational patents | `lifescience-patent-intelligence` |
| Biological sequence search result analysis, sequence patent landscape | `lifescience-sequence-report` |
| Chemical structure search result analysis, structure patent landscape | `lifescience-structure-report` |
...
|----------------|---------------------|
| Target (e.g., "PCSK9 inhibitor analysis") | `lifescience-target-intelligence` + `lifescience-commercial-analysis` |
| Drug (e.g., "analyze semaglutide") | `lifescience-pharmaceuticals-exploration` + `lifescience-clinical-outcome-analysis` + `lifescience-commercial-analysis` |
| Disease (e.g., "NSCLC treatment landscape") | `lifescience-disease-investigation` + `lifescience-epidemiology-analysis` + `lifescience-commercial-analysis` |
| Company (e.g., "AstraZeneca pipeline analysis") | `lifescience-company-profiling` + `lifescience-deal-intelligence` |
| Target + Company | `lifescience-target-intelligence` + `lifescience-company-profiling` + `lifescience-commercial-analysis` |
| Target + Disease | `lifescience-target-intelligence` + `lifescience-disease-investigation` + `lifescience-clinical-outcome-analysis` |
...
Skills needed:
├── lifescience-company-profiling   → Pfizer's CAR-T assets and pipeline
├── lifescience-patent-intelligence → CAR-T patent landscape, Pfizer IP position
└── lifescience-target-intelligence → competitive landscape for CAR-T space

Execution order:
1. `lifescience-company-profiling` (primary anchor — establish Pfizer's CAR-T assets)
2. `lifescience-patent-intelligence` + `lifescience-target-intelligence` (parallel — independent of each other, both use company context)
```
...
- [ ] **If the query is a broad analysis request (no explicit dimension restriction), have I applied the Principle 2 default bundle?** (e.g., Target query → `lifescience-target-intelligence` + `lifescience-commercial-analysis`; Drug query → `lifescience-pharmaceuticals-exploration` + `lifescience-clinical-outcome-analysis` + `lifescience-commercial-analysis`)
...
PROHIBITED: Routing a broad Target analysis query (e.g., "PCSK9 inhibitor analysis", "EGFR竞争格局") to `lifescience-target-intelligence` alone — Principle 2 default bundle requires `lifescience-target-intelligence` + `lifescience-commercial-analysis`.
...
|-------|------------------------|
| `lifescience-target-intelligence` | ls_target_fetch → ls_paper_search/fetch → ls_drug_search/fetch → ls_drug_deal_search/fetch → ls_clinical_trial_search/fetch → ls_clinical_trial_result_search/fetch → ls_patent_search/fetch → ls_patent_vector_search → ls_news_vector_search/fetch → ls_antibody_antigen_search* → ls_web_search* |
| `lifescience-pharmaceuticals-exploration` | ls_drug_search/fetch → ls_paper_search/fetch → ls_translational_medicine_search/fetch → ls_clinical_trial_search/fetch → ls_clinical_trial_result_search/fetch → ls_fda_label_vector_search → ls_clinical_guideline_vector_search → ls_drug_deal_search/fetch → ls_web_search* |
| `lifescience-disease-investigation` | ls_disease_fetch → ls_paper_search/fetch → ls_translational_medicine_search/fetch → ls_epidemiology_vector_search → ls_clinical_guideline_vector_search → ls_clinical_trial_search/fetch → ls_drug_search/fetch → ls_drug_deal_search/fetch |
| `lifescience-company-profiling` | ls_organization_fetch → ls_financial_report_vector_search → ls_drug_search/fetch → ls_drug_deal_search/fetch → ls_patent_search/fetch → ls_clinical_trial_search/fetch → ls_clinical_trial_result_search/fetch → ls_news_vector_search/fetch → ls_web_search* |
| `lifescience-deal-intelligence` | ls_drug_deal_search/fetch → ls_drug_search/fetch → ls_organization_fetch → ls_patent_search/fetch → ls_financial_report_vector_search → ls_news_vector_search/fetch → ls_web_search* |
| `lifescience-epidemiology-analysis` | ls_disease_fetch → ls_epidemiology_vector_search → ls_translational_medicine_search/fetch → ls_paper_search/fetch → ls_clinical_trial_search → ls_drug_search/fetch |
| `lifescience-commercial-analysis` | ls_drug_search/fetch → ls_epidemiology_vector_search → ls_clinical_guideline_vector_search → ls_drug_deal_search/fetch → ls_organization_fetch → ls_financial_report_vector_search → ls_web_search* |
| `lifescience-catalyst-events` | ls_drug_search/fetch → ls_clinical_trial_search/fetch → ls_clinical_trial_result_search/fetch → ls_organization_fetch → ls_news_vector_search/fetch → ls_financial_report_vector_search → ls_web_search* |
| `lifescience-regulatory-analysis` | ls_drug_search/fetch → ls_fda_label_vector_search → ls_clinical_trial_search/fetch → ls_clinical_trial_result_search/fetch → ls_clinical_guideline_vector_search → ls_news_vector_search/fetch → ls_web_search* |
| `lifescience-biomarker-analysis` | ls_paper_search/fetch → ls_target_fetch → ls_translational_medicine_search/fetch → ls_clinical_trial_search/fetch → ls_clinical_trial_result_search/fetch → ls_drug_search/fetch → ls_drug_deal_search/fetch → ls_fda_label_vector_search → ls_patent_search/fetch → ls_patent_vector_search* → ls_antibody_antigen_search* → ls_news_vector_search/fetch → ls_web_search* |
| `lifescience-clinical-outcome-analysis` | ls_drug_search/fetch → ls_clinical_trial_search/fetch → ls_clinical_trial_result_search/fetch → ls_paper_search/fetch → ls_paper_vector_search* → ls_translational_medicine_search/fetch → ls_fda_label_vector_search → ls_clinical_guideline_vector_search → ls_web_search* |
| `lifescience-patent-intelligence` | ls_patent_search/fetch → hybrid_search* → ls_patent_vector_search → ls_drug_search/fetch → ls_organization_fetch → ls_news_vector_search/fetch → ls_sequence_search_submit/poll/fetch* → ls_web_search* |
| `lifescience-pharmacovigilance` | ls_drug_search/fetch → ls_fda_label_vector_search → ls_clinical_trial_result_search/fetch → ls_paper_search/fetch |
| `lifescience-precision-oncology` | ls_drug_search/fetch → ls_clinical_trial_search/fetch → ls_clinical_trial_result_search/fetch → ls_fda_label_vector_search → ls_paper_search/fetch |
| `lifescience-gwas-target-discovery` | ls_drug_search/fetch → ls_clinical_trial_search/fetch → ls_drug_deal_search/fetch → ls_patent_search/fetch |
| `lifescience-sequence-report` | ls_sequence_search_submit → ls_sequence_search_check_status → ls_sequence_search_get_results → ls_patent_fetch → ls_drug_search/fetch → ls_organization_fetch |
| `lifescience-structure-report` | ls_chemical_search → ls_patent_fetch → ls_chemical_mcs_analyze* → ls_drug_search/fetch → ls_paper_search/fetch → ls_organization_fetch → ls_web_search* |
| `lifescience-general-research` | **Standard**: adaptive — use entity-type table in skill body. **Time-bounded query** (query contains time constraint + no single specific entity): Step 1 [MANDATORY] resolve dates → Step 2 [MANDATORY] ls_news_vector_search/fetch per domain → Step 3 [MANDATORY] ls_clinical_trial_search/fetch + ls_drug_deal_search/fetch + ls_clinical_trial_result_search/fetch with date params → Step 4 [MANDATORY for ≤30d] ls_web_search → Step 5 synthesize. All 5 steps must appear in the Tool Checklist. |
...
[ ] 1. lifescience-target-intelligence — EGFR pipeline, competitive landscape, patent analysis
    Tool Checklist:
...
[x] 1. lifescience-target-intelligence — complete (all mandatory tools called)
...
Routing: Apply Principle 2 default bundle for Target entity:
  → `lifescience-target-intelligence` + `lifescience-commercial-analysis`

Execution Plan: PCSK9 inhibitor comprehensive analysis
[ ] 1. lifescience-target-intelligence — PCSK9 competitive landscape, pipeline, patents
    Tool Checklist:
...
[ ] 2. lifescience-commercial-analysis — PCSK9 market size, pricing, reimbursement
    Tool Checklist:
...
Routing: `lifescience-general-research` → Time-Bounded Aggregation Mode (all 5 steps)

Time window resolved: 最近一周 → 2026-04-06 ~ 2026-04-13

Execution Plan: Oncology + Autoimmune pipeline updates past 7 days
[ ] 1. lifescience-general-research — Time-Bounded Aggregation Mode
    Tool Checklist:
...
[ ] 1. lifescience-disease-investigation — NSCLC resistance mechanisms, identify emerging targets
    Tool Checklist:
...
[ ] 2. lifescience-target-intelligence — competitive landscape for identified resistance targets
...
[ ] 3. lifescience-clinical-outcome-analysis — efficacy data for resistance-targeting drugs
    Tool Checklist:
...
Delegate To: lifescience-company-profiling + lifescience-target-intelligence
```
...
|-------|-----------------|
| `lifescience-target-intelligence` | ls_target_fetch → ls_drug_search/fetch → ls_drug_deal_search/fetch → ls_news_vector_search/fetch |
| `lifescience-pharmaceuticals-exploration` | ls_drug_search/fetch → ls_clinical_trial_search/fetch → ls_clinical_trial_result_search/fetch |
| `lifescience-disease-investigation` | ls_disease_fetch → ls_paper_search/fetch → ls_clinical_guideline_vector_search |
| `lifescience-company-profiling` | ls_organization_fetch → ls_drug_search/fetch → ls_news_vector_search/fetch |
| `lifescience-deal-intelligence` | ls_drug_deal_search/fetch → ls_organization_fetch |
| `lifescience-epidemiology-analysis` | ls_disease_fetch → ls_epidemiology_vector_search |
| `lifescience-commercial-analysis` | ls_drug_search/fetch → ls_epidemiology_vector_search → ls_financial_report_vector_search |
| `lifescience-regulatory-analysis` | ls_drug_search/fetch → ls_fda_label_vector_search → ls_news_vector_search/fetch |
| `lifescience-biomarker-analysis` | ls_paper_search/fetch → ls_target_fetch → ls_drug_search/fetch |
| `lifescience-clinical-outcome-analysis` | ls_drug_search/fetch → ls_clinical_trial_result_search/fetch → ls_paper_search/fetch |
| `lifescience-patent-intelligence` | ls_patent_search/fetch → ls_patent_vector_search |
| `lifescience-pharmacovigilance` | ls_drug_search/fetch → ls_fda_label_vector_search → ls_clinical_trial_result_search/fetch |
| `lifescience-precision-oncology` | ls_drug_search/fetch → ls_clinical_trial_result_search/fetch → ls_fda_label_vector_search |
| `lifescience-gwas-target-discovery` | ls_drug_search/fetch → ls_clinical_trial_search/fetch |
| `lifescience-general-research` | **Standard (non-time-bounded)**: adaptive — use entity-type table in skill body, high-priority paths only. **⛔ Time-Bounded Aggregation queries**: Fast-track is PROHIBITED — must use Deep-dive + full 5-step Time-Bounded Aggregation Mode. |
...
```
Fallback: lifescience-general-research
Trigger conditions:
...
**Execution Plan:**
1. `lifescience-company-profiling` (Primary)
2. `lifescience-target-intelligence` (Secondary)
...
**Connection URL**: `https://connect.patsnap.com/096456/mcp?apikey=sk-0qTkLefyInR5fKP3ZPQdLKg2Jbxn8o7fRyikYpsQir1vI6uV`
...
|------|--------|---------------|
| **Zone 1** Commercial | `lifescience-deal-intelligence`, `lifescience-company-profiling`, `lifescience-patent-intelligence`, `lifescience-commercial-analysis` | **P only** |
| **Zone 2** Clinical | `lifescience-clinical-outcome-analysis`, `lifescience-regulatory-analysis`, `lifescience-epidemiology-analysis` | **P primary + selective S/E** |
| **Zone 3** Scientific | `lifescience-target-intelligence`, `lifescience-disease-investigation`, `lifescience-pharmaceuticals-exploration`, `lifescience-biomarker-analysis` | **P + S co-equal** |
| **Zone 4** Computational | `lifescience-pharmacovigilance`, `lifescience-precision-oncology`, `lifescience-gwas-target-discovery` | **E/C/S primary + P context** |
...
executes_inline:
  - "lifescience-target-intelligence"
  - "lifescience-pharmaceuticals-exploration"
  - "lifescience-disease-investigation"
  - "lifescience-company-profiling"
  - "lifescience-deal-intelligence"
  - "lifescience-epidemiology-analysis"
  - "lifescience-commercial-analysis"
  - "lifescience-regulatory-analysis"
  - "lifescience-biomarker-analysis"
  - "lifescience-clinical-outcome-analysis"
  - "lifescience-patent-intelligence"
  - "lifescience-pharmacovigilance"
  - "lifescience-precision-oncology"
  - "lifescience-gwas-target-discovery"
  - "lifescience-general-research"
```
