...
description: >-
  ...
  SVG by default. Trigger on: "画图" "帮我画" "生成图" "做个图" "架构图" "流程图"
...
Generate production-quality SVG technical diagrams.
...
### 1. `generate-diagram.sh` - Validate SVG + optional PNG export
```bash
./scripts/generate-diagram.sh -t architecture -s 1 -o ./output/arch.svg
```
- Validates an existing SVG file
- Exports PNG after validation when PNG is explicitly requested
...
9. **Default output**: Return the SVG path only
10. **Optional outputs**: Generate PNG or JSON only if the user explicitly asks for them, or if the SVG-only path is insufficient for the task
11. **Report** only the files that were actually generated
