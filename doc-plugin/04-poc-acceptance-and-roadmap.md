...
本节列出 POC 过程中常用的 CLI 命令。所有命令都在项目根目录执行。

通用说明：

- `PYTHONPATH=plugin-poc`：把 `plugin-poc/` 加入 Python 模块搜索路径，让 `python -m plugin_poc.cli` 能直接加载当前仓库里的 POC 代码。
- `python -m plugin_poc.cli`：进入 Plugin POC 的统一 CLI 入口。
- `--registry /tmp/plugin-poc-registry`：模拟 Plugin Registry，用本地目录保存已发布的插件包。
- `--state /tmp/plugin-poc-state`：模拟平台状态存储，用本地目录保存安装、启用、凭据、能力索引、审计和事件记录。
- `company.slack-demo`：示例插件 ID，来自 `plugin-poc/examples/slack-demo/plugin.yaml`。
- `1.0.0`：示例插件版本。
- `ws_001`、`agent_001`、`user_001`：模拟 workspace、agent 和 user，用于验证作用域绑定、权限检查和审计记录。
...
这三条命令对应插件进入平台前的开发者侧流程。

| 命令 | 作用 | 关键参数 | 输出 / 结果 |
| --- | --- | --- | --- |
| `validate` | 校验插件目录是否合法，包括 `plugin.yaml`、能力声明、schema、目录结构等 | `plugin-poc/examples/slack-demo` 是待校验插件目录 | 校验通过则说明插件包结构可被平台识别；失败会返回具体字段或结构错误 |
| `package` | 将插件目录打包成可分发的插件包 | `--output /tmp/plugin-poc-dist` 指定打包产物目录 | 在输出目录生成插件包，用于模拟后续发布或分发 |
| `publish` | 将插件发布到本地模拟 Registry | `--registry /tmp/plugin-poc-registry` 指定 Registry 目录；`--force` 表示允许覆盖同版本包 | Registry 中出现 `company.slack-demo@1.0.0` 对应的插件包和元数据 |

这一步只完成“插件可发布”，还没有安装到某个 workspace 或 agent。
...
这两条命令对应平台管理面流程：先安装插件，再把插件启用给指定 workspace 和 agent。

| 命令 | 作用 | 关键参数 | 输出 / 结果 |
| --- | --- | --- | --- |
| `install` | 从 Registry 安装指定插件版本到平台状态目录 | `company.slack-demo` 是插件 ID；`--version 1.0.0` 是插件版本；`--registry` 是读取来源；`--state` 是写入目标 | 在 `--state` 下写入已安装插件状态，表示平台已经管理该插件版本 |
| `enable` | 将已安装插件启用并绑定到指定 workspace 和 agent | `--workspace ws_001`、`--agent agent_001` 表示启用范围 | 写入启用状态，并生成或更新 Capability Index，让该 agent 可以发现插件能力 |

安装只表示平台拥有这个插件；启用才表示某个 workspace / agent 可以使用它。
...
该命令模拟业务 Agent 在运行前查询自己可用的插件能力。

| 命令 | 作用 | 关键参数 | 输出 / 结果 |
| --- | --- | --- | --- |
| `list-capabilities` | 从 Capability Index 中查询指定 workspace / agent 被授权使用的能力 | `--state` 读取平台状态；`--workspace` 和 `--agent` 决定查询作用域 | 返回 tool、OpenAPI、MCP、skill、data source 等能力列表 |

如果插件没有执行 `enable`，或者 workspace / agent 不匹配，这里不应该看到对应能力。
...
| 命令 | 作用 | 关键参数 | 输出 / 结果 |
| --- | --- | --- | --- |
| `configure-credential` | 为指定插件版本和 workspace 写入凭据配置 | `--values` 是凭据字段 JSON；示例里写入 `client_id` 和 `client_secret` | 凭据被保存到 `--state` 中，后续调用能力时由 Credential Broker 检查和注入 |

POC 中凭据只是本地状态和脱敏展示，生产化后应接入公司密钥系统并加密存储。
...
该命令模拟业务 Agent 调用一个插件 tool capability。

| 命令 | 作用 | 关键参数 | 输出 / 结果 |
| --- | --- | --- | --- |
| `invoke` | 通过统一调用网关调用指定 capability | `company.slack-demo.tool.send_message` 是 capability ID；`--workspace`、`--agent`、`--user` 用于权限和审计；`--input` 是工具输入参数 | 返回统一 Invocation Result，并写入 audit log / runtime event |
...
`--confirm-sensitive` 用于模拟敏感操作确认。没有确认时，命令可用于验证敏感操作是否会被策略拦截。
...
该命令模拟 Agent 获取 Skill 上下文。

| 命令 | 作用 | 关键参数 | 输出 / 结果 |
| --- | --- | --- | --- |
| `render-skill-context` | 将当前 workspace / agent 可用的 Skill 渲染成可拼入 Agent 上下文的文本 | `--workspace`、`--agent` 决定加载哪些已授权 Skill | 输出 Skill 说明、任务步骤、约束和工具使用建议 |

Skill 不等同于 tool 调用。它更像 Agent 的任务方法说明，通常被放入 system prompt、developer prompt 或 agent context。
...
这两条命令用于验证 Runtime Host 的生命周期和健康检查模型。

| 命令 | 作用 | 关键参数 | 输出 / 结果 |
| --- | --- | --- | --- |
| `start-runtime` | 启动或登记指定插件版本的 runtime 状态 | `company.slack-demo` 和 `--version 1.0.0` 指定插件 runtime；`--state` 写入状态 | 在 POC 状态中记录 runtime 已启动，并登记 stdio adapter metadata |
| `runtime-health` | 查询指定插件 runtime 的健康状态 | `--plugin-id`、`--version` 指定查询对象 | 返回 runtime health 信息，用于验证平台能管理插件运行态 |

当前 POC 中 Runtime Host 只是生命周期状态模型，没有真正启动子进程、容器或 sidecar。它用于验证未来运行边界和管理接口。
...
这两条命令用于查看调用链路产生的治理记录。

| 命令 | 作用 | 关键参数 | 输出 / 结果 |
| --- | --- | --- | --- |
| `list-audit` | 查看审计日志 | `--state` 指向平台状态目录 | 输出谁在什么 workspace / agent / user 作用域下调用了什么能力，以及调用结果 |
| `list-events` | 查看 runtime event 日志 | `--state` 指向平台状态目录 | 输出 runtime 调用、健康检查、超时等运行事件 |

Audit 更偏安全和合规视角，回答“谁调用了什么”；Events 更偏运行态视角，回答“runtime 发生了什么”。
