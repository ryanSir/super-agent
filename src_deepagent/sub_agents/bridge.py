...
将现有 Workers 和 Skills 包装为 deepagents 兼容的工具函数。
桥接工具持有 Worker 实例引用（进程内），不产生额外网络开销。
主 Agent 和 Sub-Agent 共享同一批桥接工具实例。
"""
...
def create_worker_tools(workers: dict[str, Any]) -> list[Callable]:
    ...
    async def plan_and_decompose(
        ...
        from src_deepagent.orchestrator.prompts.planning import PLANNING_PROMPT
