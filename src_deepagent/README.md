...
│   ├── custom_agents.py             # 自定义 Agent 注册表（扫描 agents/ 目录）
...
│           ├── role.md              # 角色定义
│           ├── runtime_context.md   # 运行时上下文（System Content）
...
│   ├── bridge.py                    # Worker → Agent 工具适配层（10 个工具函数）
...
```
LLM 看到的 Tool Schema    →    agent_tools 函数（bridge.py）   →    Worker 实例
─────────────────────         ──────────────────────          ──────────────
