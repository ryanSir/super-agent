...
LLM 工具接口（面向 LLM，语义化参数）：
  async def baidu_search(ctx, query: str, count: int = 10) -> dict
...
| 角色 | 职责 | 可用工具 |
|------|------|---------|
| researcher | 信息检索与综合分析 | skill, search_skills, sandbox, baidu_search |
| analyst | 数据分析与可视化 | sandbox, emit_chart, baidu_search |
...
model: gpt-4o-mini
tools: [execute_sandbox, baidu_search]
---
